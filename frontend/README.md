...
- **lint / 型チェック / フォーマット**の「通知」「ブロック」の意味は他の項目と異なる:
  - `stop`（通知）: `verify.sh`内で`vp check`（`--fix`なし）を実行。自動修正せず、失敗した内容をそのまま全文通知する
  - pre-commit（ブロック）: `vp lint --fix`で自動修正可能なものは先に修正してしまうため、**自動修正できなかったエラーのみ**がコミットをブロックする。つまり`stop`で通知された内容の一部は、pre-commit到達時には既に自動修正されて消えている場合がある
