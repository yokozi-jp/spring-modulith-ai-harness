...
**`verify.sh`は`stop`フック経由で自動実行される。** 実行タイミングごとに検証範囲が異なる。`stop`と人間のコミット時は`verify.sh`と同一のチェック（5種のshellチェックすべて）を実行するが、`postToolUse`はテスト未作成チェックのみの単独実装であり、他の4種（features構造・Hook配置・UI編集検出・API編集検出）は含まない。

| タイミング                   | 仕組み                                                   | 検証範囲                                                                                                                                                                                              | 検出漏れ                                                                                                                         |
| ---------------------------- | -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| Kiro CLI: ファイル書き込み前 | `preToolUse(write)` フック（`frontend-write-guard.sh`）  | `components/ui/`・`src/api/`への書き込みをブロック。`src/features/*`書き込み時は`check-features-structure.sh --file`、`use-*.ts`書き込み時は`check-hook-location.sh --file`を実行し、違反時はブロック | **テスト未作成のみ検証しない**（書き込み前で対象ファイルが未確定のため技術的に不可能）。lint/型/フォーマットも同様の理由で未検証 |
| Kiro CLI: ファイル書き込み後 | `postToolUse(write)` フック（`frontend-test-prompt.sh`） | **テスト未作成のみ**を検証（`src/features/*/hooks/`・`src/hooks/`・`src/lib/`、`check-test-exists.sh`と同一範囲）。通知のみでブロックしない                                                           | 他4種（features構造・Hook配置・UI編集検出・API編集検出）は非対応。lint/型/フォーマットも非対応                                   |
...
つまり`components/ui/`・`src/api/`の誤編集は書き込み前にブロックされ、テスト未作成は書き込み後に即座に通知される。この2種は「AIの書き込み中に検出できない」状態を解消済みだが、残る3種（features構造・Hook配置以外の、lint/型/フォーマット全体）は`stop`フックまで検出が遅れる。`stop`フックは`verify.sh`をそのまま実行するため最終防衛線としては網羅的だが、ブロック機能を持たない（exit codeでの強制は`preToolUse`のみ可能）ため、この通知を無視して応答を終えることは技術的に可能である。steeringの「コード変更後は必ず`./scripts/verify.sh`を実行する」という要求と`stop`フックの検証内容は一致している。
