...
| ----------------------------------------------- | :--------------------------: | :---------------------------: | :--------------------: | :------------------------: |
| lint / 型チェック / フォーマット（`vp check`）  |              ❌              |              ❌               |           ✅           |             ✅             |
| features/ 構造（`check-features-structure.sh`） |         ✅ ブロック          |              ❌               |           ✅           |        ✅ ブロック         |
| Hook配置（`check-hook-location.sh`）            |         ✅ ブロック          |              ❌               |           ✅           |        ✅ ブロック         |
| components/ui/ 誤編集（`check-ui-readonly.sh`） |         ✅ ブロック          |              ❌               |           ✅           |        ✅ ブロック         |
| src/api/ 誤編集（`api-readonly.sh`）            |         ✅ ブロック          |              ❌               |           ✅           |        ✅ ブロック         |
| テスト未作成（`check-test-exists.sh`）          |              ❌              |            ✅ 通知            |           ✅           |        ✅ ブロック         |

凡例:

- **✅ ブロック**: 違反があれば操作自体を止める（書き込み拒否 / コミット拒否）
- **✅ 通知**: 違反があればAIに伝えるが、操作は止めない
- **✅**（`stop`列）: `./scripts/verify.sh`をそのまま実行するため全項目が対象。ただし通知のみでブロックはできない（`stop`フックの仕様上の制約、ブロック可能なのは`preToolUse`のみ）
- **❌**: このタイミングでは検証されない
...
- 応答終了時の`stop`フックは全項目を検証する最終防衛線だが、ブロックできないため、AIが通知を無視して応答を終えることは技術的に可能
