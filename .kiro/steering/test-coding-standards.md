...
|---|---|---|---|
| （タグなし） | `make test-only T='*SampleTest'` | なし（Docker 不要） | ✅ |
| `@Tag("integration")` | `make test-only T='*RepositoryImplTest'` | PostgreSQL コンテナ | ✅ |
| `@Tag("e2e")` | `make test-only T='*E2eTest'` | 全コンテナ（PostgreSQL + Redis + Keycloak） | ❌（手動実行） |

### TDD ワークフロー

```
1. Domain の Red-Green     → make test-only T='*SampleTest'            (即座)
2. Handler の Red-Green    → make test-only T='*CommandHandlerTest'     (即座)
3. Controller の Red-Green → make test-only T='*ControllerTest'         (即座)
4. DB 結合確認             → make test-only T='*RepositoryImplTest'     (DB起動)
5. 全体確認                → make test                                  (全コンテナ)
```
...
| # | テスト対象 | アノテーション | 外部依存 | 何を検証するか | scaffold type |
|---|---|---|---|---|---|
| 5a | API E2E（RANDOM_PORT） | `@SpringBootTest(RANDOM_PORT)` + `@Tag("e2e")` | PostgreSQL + Redis + Keycloak | HTTP リクエスト→レスポンスの完全フロー | `e2e` |
| 5b | コンテキストロード | `@SpringBootTest` + `@Tag("e2e")` | 全部 | Bean 定義の整合性 | （プロジェクト共通、既存） |
| 5c | Observability | `@SpringBootTest(RANDOM_PORT)` + `@Tag("e2e")` | OTEL Collector | トレース・メトリクス送信 | （プロジェクト共通） |
...
| `e2e` | `src/test/` | `@SpringBootTest(RANDOM_PORT)` + `@Tag("e2e")` | Controller（HTTP完全フロー） |
...
- `make test` で全チェック、`make test-only T='*ClassName'` で特定テストを実行
