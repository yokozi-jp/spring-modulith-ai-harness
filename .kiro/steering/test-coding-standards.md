...
|---|---|---|---|
| （タグなし） | `./gradlew test` | なし（Docker 不要） | ✅ |
| `@Tag("integration")` | `./gradlew integrationTest` | PostgreSQL コンテナ | ✅ |
| `@Tag("e2e")` | `./gradlew e2eTest` | 全コンテナ（PostgreSQL + Redis + Keycloak） | ❌（手動実行） |

### TDD ワークフロー

```
1. Domain の Red-Green     → ./gradlew test --tests '*SampleTest'            (即座)
2. Handler の Red-Green    → ./gradlew test --tests '*CommandHandlerTest'     (即座)
3. Controller の Red-Green → ./gradlew test --tests '*ControllerTest'         (即座)
4. DB 結合確認             → ./gradlew integrationTest --tests '*RepositoryImplTest'  (DB起動)
5. 全体確認                → ./gradlew e2eTest                                (全コンテナ)
```

### テストディレクトリ構成

**本番コードのモジュール構成を `src/test/java` でそのままミラーする。**
テスト種別（unit / integration / e2e）で横断ディレクトリを切らない。

```
src/test/java/com/example/demo/
├── architecture/                    ← ArchUnit / Modulith 境界検証
├── testconfig/                      ← テストサポート（ContainerConfig 等）
├── DemoApplicationTests.java        ← @Tag("e2e") コンテキストロード
├── GlobalExceptionHandlerTest.java  ← unit グローバル例外ハンドラ
├── LiquibaseMigrationTest.java      ← @Tag("integration") マイグレーション検証
├── ActuatorSecurityTest.java        ← @Tag("e2e") Actuator Basic 認証
├── OAuth2AuthenticationTest.java    ← @Tag("e2e") OAuth2 リダイレクト検証
├── ObservabilityTest.java           ← @Tag("e2e") OTEL SDK 検証
├── sample/
...
| 2c | jOOQ Query（DB スライス） | `@JooqTest` + PostgresContainerConfig | PostgreSQL | SQL クエリの正しさ、ページネーション | `jooqquery` |
| 2d | Liquibase マイグレーション | `@SpringBootTest` + PostgresContainerConfig | PostgreSQL | スキーマ適用・ロールバック | （プロジェクト共通） |

### 3. Integration テスト（複数レイヤー結合）

| # | テスト対象 | アノテーション | 外部依存 | 何を検証するか | scaffold type |
|---|---|---|---|---|---|
| 3a | RepositoryImpl | `@SpringBootTest` + PostgresContainerConfig | PostgreSQL | CRUD、ID生成、jOOQ マッピング | `integration` |
| 3b | QueryServiceImpl | `@SpringBootTest` + PostgresContainerConfig | PostgreSQL | 検索・ページネーション・フィルタ | `integration` |
| 3c | UseCase → Domain 結合 | `@SpringBootTest` + PostgresContainerConfig | PostgreSQL | CommandHandler → Factory → Repository → DB の一連フロー | `usecase` |
...
### 5. E2E / Full-Stack テスト（全コンテナ起動）

| # | テスト対象 | アノテーション | 外部依存 | 何を検証するか | scaffold type |
|---|---|---|---|---|---|
| 5a | API E2E（RANDOM_PORT） | `@SpringBootTest(RANDOM_PORT)` + FullStackContainerConfig | PostgreSQL + Redis + Keycloak | HTTP リクエスト→レスポンスの完全フロー | `e2e` |
| 5b | コンテキストロード | `@SpringBootTest` + FullStackContainerConfig | 全部 | Bean 定義の整合性 | （プロジェクト共通、既存） |
| 5c | Observability | `@SpringBootTest(RANDOM_PORT)` + ObservabilityContainerConfig | OTEL Collector | トレース・メトリクス送信 | （プロジェクト共通） |
...
| `integration` | `src/test/` | `@SpringBootTest` + `@Import(PostgresContainerConfig.class)` + `@Tag("integration")` | RepositoryImpl, QueryServiceImpl |
| `usecase` | `src/test/` | `@SpringBootTest` + `@Import(PostgresContainerConfig.class)` + `@Tag("integration")` | CommandHandler（UseCase→DB結合） |
| `moduletest` | `src/test/` | `@ApplicationModuleTest` + `Scenario` + `@Tag("integration")` | CommandHandler（イベント発行・購読） |
| `jooqquery` | `src/test/` | `@JooqTest` + `@Import(PostgresContainerConfig.class)` + `@Tag("integration")` | QueryServiceImpl（SQL クエリ検証） |
| `e2e` | `src/test/` | `@SpringBootTest(RANDOM_PORT)` + `@Import(FullStackContainerConfig.class)` + `@Tag("e2e")` | Controller（HTTP完全フロー） |
...
- テストサポートクラス（`PostgresContainerConfig` 等）はこの限りでない
...
テストコンテナ設定は `src/test/java/com/example/demo/testconfig/` に配置する。

| クラス | 用途 |
|---|---|
| `PostgresContainerConfig` | PostgreSQL のみ（`@Tag("integration")` テストで使用） |
| `RedisContainerConfig` | Redis のみ |
| `KeycloakContainerConfig` | Keycloak のみ |
| `ObservabilityContainerConfig` | OTEL Collector |
| `FullStackContainerConfig` | 全コンテナ合成（`@Tag("e2e")` テストで使用） |
...
| `PMD.TestClassWithoutTestCases` | テストサポートクラス（`PostgresContainerConfig` 等） | クラスレベル |
