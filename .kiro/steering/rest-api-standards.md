...
- ベースパス: `/api/v{version}/` — `WebMvcConfig` が全 `@RestController` に自動付与
- リソース名は複数形: `/orders`, `/products`
- パス変数は ID: `/orders/{id}`
...
Spring Framework 7 の `ApiVersionConfigurer` + パスセグメント方式を使用する。

- 設定: `com.example.demo.config.WebMvcConfig`
- パス構造: `/api/v1/orders`, `/api/v2/orders`
...
public OrderDetailResponseV1 findByIdV1(@PathVariable final String id) { ... }

@GetMapping(value = "/{id}", version = "2+")
public OrderDetailResponseV2 findByIdV2(@PathVariable final String id) { ... }
```

- `version = "1"` — v1 のみマッチ
- `version = "2+"` — v2 以降すべてにマッチ（ベースライン）
- `version` 未指定 — 全バージョンにマッチ（最低優先度、フォールバック）
- サポート外バージョン → 自動で `400 Bad Request`
